    ...
    for epoch in range(opt.niter):
        ...
        for j in range(opt.Dsteps):
            ...
            if (j == 0) & (epoch == 0):
                if (Gs == []) & (opt.mode != 'SR_train'):
                    ...
                    # Our adaptation
                    # L1Loss, MSELoss, SmoothL1Loss, NLLLoss, CrossEntropyLoss, KLDivLoss
                    if opt.loss_function == 'MSELoss':
                        criterion = nn.MSELoss()
                        loss = torch.sqrt(criterion(real, z_prev))
                    elif opt.loss_function == 'L1Loss':
                        criterion = nn.L1Loss()
                        loss = criterion(real, z_prev)
                    elif opt.loss_function == 'SmoothL1Loss':
                        criterion = nn.SmoothL1Loss()
                        loss = criterion(real, z_prev)
                    elif opt.loss_function == 'NLLLoss':
                        criterion = nn.NLLLoss()
                        loss = criterion(real, z_prev)
                    elif opt.loss_function == 'CrossEntropyLoss':
                        criterion = nn.CrossEntropyLoss()
                        loss = criterion(real, z_prev)
                    elif opt.loss_function == 'KLDivLoss':
                        criterion = nn.KLDivLoss()
                        loss = criterion(real, z_prev)
                    else:
                        raise KeyError("Wrong loss function. Please Choose from list (see --help).")
                    # criterion = nn.MSELoss()
                    # RMSE = torch.sqrt(criterion(real, z_prev))
                    # opt.noise_amp = opt.noise_amp_init * RMSE
                    opt.noise_amp = opt.noise_amp_init * loss
                    z_prev = m_image(z_prev)
            else:
                prev = draw_concat(Gs, Zs, reals, NoiseAmp, in_s, 'rand', m_noise, m_image, opt)
                prev = m_image(prev)
        ...
        for j in range(opt.Gsteps):
            ...
            if alpha != 0:
                ...
                z_prev = z_prev.float()
                Z_opt_float = Z_opt.detach().float()
                tensor = loss(netG(Z_opt_float, z_prev), real)
