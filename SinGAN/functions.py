...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        dir2save = 'TrainedModels/%s/scale_factor=%f,alpha=%d,loss_function=%s' % (opt.input_name[:-4], opt.scale_factor_init, opt.alpha, opt.loss_function)
        ...
        dir2save = '%s/RandomSamples/%s/gen_start_scale=%d,loss_function=%s' % (opt.out, opt.input_name[:-4], opt.gen_start_scale, opt.loss_function)
